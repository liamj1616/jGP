...
public class Printer
{
	...
	public static void printLineAllRegressors(String directory, int runNum, Chromosome[] allToPrint)
	{
		try
		{
			Writer outFile = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(directory + "line_python_" + runNum + ".py"), "utf-8"));
			outFile.write("from math import * \n");
			for(int i = allToPrint.length - 1 ; i >= 0 ; i--)
			{
				outFile.write("def symFunction" + (i+1) + "_" + (i+2) + "(x): return ");
				traversePrintLine(allToPrint[i].numGenes - 1, allToPrint[i], outFile);
				outFile.write("\n");
			}

			outFile.write("all_gen_functions = [");
			for(int i = allToPrint.length - 1 ; i >= 0 ; i--)
			{
				outFile.write("symFunction" + (i+1) + "_" + (i+2) + ", ");
			}

			outFile.write("]\n");
			outFile.write("def getFunctions(): return all_gen_functions\n");
			outFile.close();
		}
	...
	public static void traversePrintLine(int index, Chromosome toPrint, Writer outFile)
	{
		try
		{
			...
			{
				if(toPrint.geneArray[index].operator == "e")
				{
					outFile.write("exp" + "(");
				}
				else
			...
			{
				if(toPrint.geneArray[index].operator == "min" || toPrint.geneArray[index].operator == "max")
				{
					outFile.write(toPrint.geneArray[index].operator);
					outFile.write("(");
					traversePrintLine(toPrint.geneArray[index].ref1, toPrint, outFile);
